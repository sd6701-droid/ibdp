# TransNetV2 architecture -- VENDORED VERBATIM, do not edit.
#
# Source: https://github.com/soCzech/TransNetV2
#         inference-pytorch/transnetv2_pytorch.py @ 85cef72
#         fetched 2026-07-29
#
# MIT License. Copyright (c) 2020 Tomas Soucek (soCzech).
# Everything below this header is unmodified upstream code.
#
# WHY VENDORED, rather than pip-installed or downloaded at run time:
#
#   1. The checkpoint in $ROOT/models/TransNetV2 is a bare state_dict with no
#      architecture attached. This file and that .pth are ONE artefact --
#      load_state_dict(strict=True) against this exact definition is the only
#      thing standing between a key mismatch and cuts that are silently wrong.
#   2. Compute nodes have no outbound internet (same reason every sbatch here
#      exports HF_HUB_OFFLINE=1), so fetching it at run time is not an option.
#
# Used by scripts/32_split_scenes.py.
import torch
