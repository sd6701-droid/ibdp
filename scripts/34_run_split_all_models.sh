...
# wrap in sbatch.
#
# No W&B on purpose: a five-clip probe is not an experiment to track, and the
# JSON is the deliverable. No resume either -- the point of a probe is a FRESH
# answer; records still append to new annotations_*.jsonl files, so nothing
# already on disk is touched, and 33 picks the newest record per (model,
# segment) when bundling.
