...
import os
import itertools
# Video-reading deps are only needed by get_fps/read_video/get_video_information_*;
# the ViTPose JSON path (convert_vitpose_json.py) never touches them, so their
# absence must not break preprocess/build_features.
try:
    import cv2
except ImportError:
    cv2 = None
...
def get_joint_angles(df):
    ...
    # numeric_only: pandas >=2 no longer silently drops the string 'bp' column,
    # and the result only needs the deduplicated group keys anyway.
...
def get_dynamics_xy(xdf, delta_window):
    ...
    xdf = xdf.groupby(['bp','video'], group_keys=False).apply(lambda x: get_delta(x,'x','d_x')).reset_index(drop=True)
    xdf = xdf.groupby(['bp','video'], group_keys=False).apply(lambda x: get_delta(x,'y','d_y')).reset_index(drop=True)
    xdf['displacement'] = np.sqrt(xdf['d_x']**2 + xdf['d_y']**2)
    xdf['velocity_x_raw'] = xdf['d_x']/xdf['delta_t']
    xdf['velocity_y_raw'] = xdf['d_y']/xdf['delta_t']
    xdf = xdf.groupby(['bp','video'], group_keys=False).apply(lambda x: smooth_dyn(x,'velocity_x_raw','velocity_x', delta_window))
    xdf = xdf.groupby(['bp','video'], group_keys=False).apply(lambda x: smooth_dyn(x,'velocity_y_raw','velocity_y', delta_window))
    xdf = xdf.groupby(['bp','video'], group_keys=False).apply(lambda x: get_delta(x,'velocity_x','delta_velocity_x')).reset_index(drop=True)
    xdf = xdf.groupby(['bp','video'], group_keys=False).apply(lambda x: get_delta(x,'velocity_y','delta_velocity_y')).reset_index(drop=True)
    xdf['acceleration_x_raw'] = xdf['delta_velocity_x']/xdf['delta_t']
    xdf['acceleration_y_raw'] = xdf['delta_velocity_y']/xdf['delta_t']
    xdf = xdf.groupby(['bp','video'], group_keys=False).apply(lambda x: smooth_dyn(x,'acceleration_x_raw','acceleration_x', delta_window))
    xdf = xdf.groupby(['bp','video'], group_keys=False).apply(lambda x: smooth_dyn(x,'acceleration_y_raw','acceleration_y', delta_window))
    ...
    xdf = xdf.groupby(['bp','video'], group_keys=False).apply(lambda x: smooth_dyn(x,'speed_raw','speed', delta_window))
    xdf['part'] = xdf.bp.str[1:]
    xdf['side'] = xdf.bp.str[:1]
    return xdf

def get_dynamics_angle(adf, delta_window):
    adf = adf.groupby(['bp','video'], group_keys=False).apply(lambda x: get_angle_displacement(x,'angle','displacement')).reset_index(drop=True)
    adf['velocity_raw'] = adf['displacement']/adf['delta_t']
    adf = adf.groupby(['bp','video'], group_keys=False).apply(lambda x: smooth_dyn(x,'velocity_raw','velocity', delta_window))
    adf = adf.groupby(['bp','video'], group_keys=False).apply(lambda x: get_delta(x,'velocity','delta_velocity')).reset_index(drop=True)
    adf['acceleration_raw'] = adf['delta_velocity']/adf['delta_t']
    adf = adf.groupby(['bp','video'], group_keys=False).apply(lambda x: smooth_dyn(x,'acceleration_raw','acceleration', delta_window))
